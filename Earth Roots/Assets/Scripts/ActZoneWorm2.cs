...
public class ActZoneWorm2 : MonoBehaviour
{
    private bool canAct;
    ...
    private void Update()
    {
        if (Variables.interactWorm2 == 0 && _canSkip)
        {
            ...
            else if (Input.GetKeyDown(KeyCode.Space) && replic == _replicas.Length)
            {
                _sounds.clip = _trigger;
                _sounds.Play();
                Variables.interactWorm2 = 1;
                TextFade();
            }
        }
        else if (Variables.interactWorm2 == 1 && _canSkip)
        {
